...
router
  ...
  .get("/", async (req, res) => {
    ...
    try {
      const rows = await mysqlPool.query("select * from COMPANY;");
      if (rows.length === 0) {
        res.status(404).send();
        return;
      }
  ...
  .post("/", async (req, res) => {
    ...
    try {
      await mysqlPool.query("insert into COMPANY set ?", user);
      const rows = await mysqlPool.query(
        "select * from  COMPANY where COMPANY_CD = ? ;",
        [user.COMPANY_CD]
      );
  ...
  .put("/", async (req, res) => {
    ...
    try {
      const rows = await mysqlPool.query(
        "update COMPANY set COMPANY_CD = ? ,COMPANY_NAME = ? where COMPANY_CD = ?",
...
router.delete("/:key1/", async (req, res) => {
  ...
  try {
    await mysqlPool.query(
      "delete from  COMPANY where COMPANY_CD = ?",
      [req.params.key1]
    );
    res.status(204).send();
  } catch (err) {
